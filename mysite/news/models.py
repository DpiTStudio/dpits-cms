...
from django.utils.text import slugify  # Функция для преобразования строки в slug (URL-дружественный формат)
from django_ckeditor_5.fields import CKEditor5Field  # Поле для расширенного текстового редактора
...
class NewsCategory(models.Model):
    ...
    # Основные поля
    name = models.CharField("Название", max_length=100)  # Название категории, максимум 100 символов
    slug = models.SlugField("URL", unique=True)  # URL-дружественный идентификатор, должен быть уникальным
    image = models.ImageField("Изображение", upload_to="news/categories/", blank=True)  # Изображение категории, может быть пустым
    description = CKEditor5Field("Описание", blank=True, config_name="extends")  # Описание категории с расширенным редактором, может быть пустым

    # СЕО поля (для поисковой оптимизации)
    seo_title = models.CharField("SEO заголовок", max_length=200, blank=True)  # SEO заголовок для поисковых систем, может быть пустым
    seo_keywords = models.CharField("SEO ключевые слова", max_length=200, blank=True)  # Ключевые слова для SEO, могут быть пустыми
    seo_description = models.CharField("SEO описание", max_length=255, blank=True)  # SEO описание для поисковых систем, может быть пустым

    # Настройки отображения
    show_in_menu = models.BooleanField("Показывать в меню", default=True)  # Показывать ли категорию в меню (по умолчанию да)
    order = models.IntegerField("Порядок", default=0)  # Порядок отображения категории в меню (чем меньше, тем выше)
    is_active = models.BooleanField("Активно", default=True)  # Активна ли категория (по умолчанию да)
    ...
    class Meta:
        ...
        ordering = ["order", "name"]  # Сортировка по порядку (возрастание) и имени (алфавитный порядок)
    ...
    def get_absolute_url(self):
        ...
        """
        return reverse("news:category", kwargs={"slug": self.slug})  # Генерируем URL используя имя маршрута и slug категории
...
class News(models.Model):
    ...
    # Основные поля
    title = models.CharField("Заголовок", max_length=200)  # Заголовок новости, максимум 200 символов
    slug = models.SlugField("URL", unique=True)  # URL-дружественный идентификатор, должен быть уникальным
    ...
    is_active = models.BooleanField("Активно", default=True)  # Активна ли новость (по умолчанию да)
    ...
    content = CKEditor5Field("Содержание", blank=True, config_name="extends")  # Полное содержание новости с расширенным редактором, может быть пустым

    # Системные поля
    views = models.PositiveIntegerField("Просмотры", default=0)  # Счетчик просмотров новости (только положительные числа, по умолчанию 0)
    created_at = models.DateTimeField("Создано", auto_now_add=True)  # Дата и время создания (устанавливается автоматически)
    updated_at = models.DateTimeField("Обновлено", auto_now=True)  # Дата и время последнего обновления (обновляется автоматически)
    ...
    class Meta:
        ...
        ordering = ["-created_at"]  # Сортировка по дате создания (новые сверху, знак минус означает убывание)
    ...
    def get_absolute_url(self):
        ...
        """
        return reverse("news:detail", kwargs={"slug": self.slug})  # Генерируем URL используя имя маршрута и slug новости
    ...
    def increment_views(self):
        ...
        self.save(update_fields=["views"])  # Сохраняем только поле views (оптимизация производительности)
